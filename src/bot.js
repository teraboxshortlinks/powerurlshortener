...
bot.setMyCommands([
    ...
    { command: 'set_channel', description: 'Add a channel for auto-posting (ID, @username, or link)' },
    { command: 'remove_channel', description: 'Remove a specific auto-posting channel' },
    { command: 'balance', description: 'Check your balance and clicks on PowerURLShortener' },
    { command: 'my_channels', description: 'Show your currently set auto-post channels' } // Command name changed
]);
 ...
 * @param {string} key - The key to store (e.g., 'token', 'header').
...
/**
 * Adds a channel to a user's list of auto-post channels.
 * Ensures no duplicate channels are added.
 * @param {number} chatId - The unique ID of the Telegram chat.
 * @param {string} channelId - The channel ID, @username, or invite link to add.
 * @returns {boolean} `true` if added, `false` if already exists.
 */
function addChannelToDatabase(chatId, channelId) {
    const db = getDatabaseData();
    if (!db[chatId]) {
        db[chatId] = {};
    }
    const channels = db[chatId]['channels'] || [];
    if (!channels.includes(channelId)) {
        channels.push(channelId);
        db[chatId]['channels'] = channels;
        fs.writeFileSync(dbPath, JSON.stringify(db, null, 2));
        return true;
    }
...
/**
 * Removes a channel from a user's list of auto-post channels.
 * @param {number} chatId - The unique ID of the Telegram chat.
 * @param {string} channelId - The channel ID, @username, or invite link to remove.
 * @returns {boolean} `true` if removed, `false` if not found.
 */
function removeChannelFromDatabase(chatId, channelId) {
    const db = getDatabaseData();
    if (!db[chatId] || !db[chatId]['channels']) {
        return false;
    }
    let channels = db[chatId]['channels'];
...
➕ \`/set_channel\` — Add auto-post channel
➕ \`/remove_channel\` — Remove a specific auto-post channel
➕ \`/my_channels\` — Show my current auto-post channels
...
// Now supports adding multiple channels to an array.
bot.onText(/\/set_channel (.+)/, async (msg, match) => {
    ...
    const added = addChannelToDatabase(msg.chat.id, inputChannel);
    if (added) {
        await sendTelegramMessage(msg.chat.id, 'text', `✅ Channel added: \`${inputChannel}\`. Please ensure I am an **administrator** in this channel with permission to post messages.`, { parse_mode: 'Markdown', isUserChat: true });
    } else {
        await sendTelegramMessage(msg.chat.id, 'text', `ℹ️ Channel \`${inputChannel}\` is already in your list.`, { parse_mode: 'Markdown', isUserChat: true });
    }
});

// Handles the /remove_channel command to remove a specific auto-posting channel.
bot.onText(/\/remove_channel (.+)/, async (msg, match) => {
    const channelToRemove = match[1].trim();
    const removed = removeChannelFromDatabase(msg.chat.id, channelToRemove);
    if (removed) {
        await sendTelegramMessage(msg.chat.id, 'text', `✅ Channel \`${channelToRemove}\` removed.`, { isUserChat: true, parse_mode: 'Markdown' });
    } else {
        await sendTelegramMessage(msg.chat.id, 'text', `ℹ️ Channel \`${channelToRemove}\` was not found in your list.`, { isUserChat: true, parse_mode: 'Markdown' });
    }
});


// Handles the /my_channels command to show the current auto-post channels. (Command name changed)
bot.onText(/\/my_channels/, async (msg) => {
    const channels = getFromDatabase(msg.chat.id, 'channels'); // Retrieve the channel settings.
    if (channels && channels.length > 0) {
        const channelList = channels.map(c => `• \`${c}\``).join('\n');
        await sendTelegramMessage(msg.chat.id, 'text', `📢 Your current auto-post channels:\n${channelList}`, { parse_mode: 'Markdown', isUserChat: true });
    } else {
        await sendTelegramMessage(msg.chat.id, 'text', 'No auto-post channels are set.', { isUserChat: true });
    }
});
...
bot.on('message', async (msg) => {
    ...
    // Also allow /set_channel and /remove_channel commands to pass through for processing.
    if (msg.text && msg.text.startsWith('/') && !msg.text.startsWith('/api') && !msg.text.startsWith('/set_channel') && !msg.text.startsWith('/remove_channel') && msg.text.length > 1 && !msg.text.includes(' ')) {
        return;
    }
    ...
    // --- MODIFIED: Get all auto-post channels ---
    const autoPostChannels = getFromDatabase(chatId, 'channels') || [];
    ...
    if (!userApiToken && !(msg.text && (msg.text.startsWith('/api') || msg.text.startsWith('/set_channel') || msg.text.startsWith('/remove_channel')))) { // Exclude `/api`, /set_channel, /remove_channel commands itself from this check.
        ...
        if (!mediaGroups[groupId]) {
            ...
            setTimeout(async () => {
                ...
                if (media.length > 0) {
                    ...
                    // --- MODIFIED: Loop through all autoPostChannels ---
                    for (const channelId of autoPostChannels) {
                        await sendTelegramMessage(channelId, 'mediaGroup', media);
                    }
                }
    ...
    if (isForwarded && msg.photo) {
        ...
        // --- MODIFIED: Loop through all autoPostChannels ---
        for (const channelId of autoPostChannels) {
            await sendTelegramMessage(channelId, 'photo', photoId, { caption: finalCaption });
        }
        return;
    }
    ...
    if (isForwarded && msg.video) {
        ...
        // --- MODIFIED: Loop through all autoPostChannels ---
        for (const channelId of autoPostChannels) {
            await sendTelegramMessage(channelId, 'video', msg.video.file_id, { caption: finalCaption });
        }
        return;
    }
    ...
    if (links.length > 0) {
        ...
        if (msg.photo) {
            ...
            // --- MODIFIED: Loop through all autoPostChannels ---
            for (const channelId of autoPostChannels) {
                await sendTelegramMessage(channelId, 'photo', photoId, { caption: finalContentWithHeaderFooter });
            }
        } else if (msg.video) {
            ...
            // --- MODIFIED: Loop through all autoPostChannels ---
            for (const channelId of autoPostChannels) {
                await sendTelegramMessage(channelId, 'video', msg.video.file_id, { caption: finalContentWithHeaderFooter });
            }
        } else {
            ...
            // --- MODIFIED: Loop through all autoPostChannels ---
            for (const channelId of autoPostChannels) {
                await sendTelegramMessage(channelId, 'text', finalContentWithHeaderFooter);
            }
        }
