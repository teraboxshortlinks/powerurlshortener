...
bot.onText(/\/start/, (msg) => {
  ...
  const welcomeMessage = `Hello, ${username}!\n\n`
    ...
    + 'You can use this bot to shorten URLs using the AdlinkFly service.\n\n'
    + 'To shorten a URL, just type or paste the URL directly in the chat, and the bot will provide you with the shortened URL.\n\n'
    + 'If you haven\'t set your AdlinkFly API token yet, use the command:\n/setapi YOUR_ADLINKFLY_API_TOKEN\n\n'
...
bot.onText(/\/setapi (.+)/, (msg, match) => {
  ...
  const response = `AdlinkFly API token set successfully. Your token: ${userToken}`;
...
async function shortenUrl(chatId, url) {
  ...
  if (!adlinkflyToken) {
    bot.sendMessage(chatId, 'Please provide your AdlinkFly API token first. Use the command: /setapi YOUR_ADLINKFLY_API_TOKEN');
    return null;
  }
